...
export default function AdminNotificationsPage() {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      if (res.ok) { toast.success(`Notification sent to ${data.count || 1} user(s).`); setForm({ target: 'all', userId: '', type: 'SYSTEM', title: '', message: '' }) }
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          {form.target === 'specific' && (
            <div className="space-y-1">
              <Label>User ID</Label>
              <Input value={form.userId} onChange={(e) => setForm({ ...form, userId: e.target.value })} placeholder="User UUID" />
            </div>
          )}
          <div className="space-y-1">
            <Label>Type</Label>
          ...
          <Button variant="gradient" onClick={handleSend} loading={sending} className="w-full">Send Notification</Button>
        </CardContent>
      </Card>
    </div>
  )
