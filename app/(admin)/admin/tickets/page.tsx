...
const STATUS_VARIANTS: Record<string, 'warning' | 'default' | 'success' | 'error'> = {
  OPEN: 'warning',
  IN_PROGRESS: 'default',
  CLOSED: 'success',
}
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div className="flex items-center gap-3">
          ...
          <div className="flex gap-2">
            ...
            {inProgressCount > 0 && <Badge variant="default">{inProgressCount} In Progress</Badge>}
          </div>
        </div>
                ...
                {loading ? (
                  ...
                  <tr key={t.id} className="hover:bg-secondary/20 transition-colors">
                    <td className="px-4 py-3">
                      <Link href={`/admin/users/${t.user.id}`} className="font-medium text-primary hover:underline text-xs">
                        {t.user.username || `${t.user.firstName ?? ''} ${t.user.lastName ?? ''}`.trim() || t.user.email}
                      </Link>
                    ...
                    <td className="px-4 py-3">
                      <Link href={`/admin/tickets/${t.id}`} className="text-primary hover:underline text-xs">View / Reply</Link>
                    </td>
                  </tr>
                ))}
              </tbody>
