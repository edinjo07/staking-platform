...
export default function AdminTicketDetailPage() {
  ...
  if (!ticket) {
    return <div className="text-center py-12 text-muted-foreground text-sm">Loading ticket...</div>
  }
  ...
  return (
    <div className="space-y-6 max-w-3xl">
      <div className="flex items-center gap-3 flex-wrap">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-sm text-muted-foreground">
            By{' '}
            <Link href={`/admin/users/${ticket.user.id}`} className="text-primary hover:underline">
        ...
        <CardContent className="p-4 space-y-4 max-h-[500px] overflow-y-auto">
          {ticket.messages.map((m) => (
            <div key={m.id} className={cn('flex gap-3', m.isStaff ? 'flex-row-reverse' : 'flex-row')}>
              <div className={cn('flex-shrink-0 h-8 w-8 rounded-full flex items-center justify-center text-xs font-bold', m.isStaff ? 'bg-primary/20 text-primary' : 'bg-secondary text-foreground')}>
                {m.isStaff ? <Shield className="h-4 w-4" /> : <User className="h-4 w-4" />}
              </div>
              <div className={cn('max-w-[75%] space-y-1', m.isStaff ? 'items-end' : 'items-start')}>
                <div className="flex items-center gap-2">
      ...
      {ticket.status !== 'CLOSED' ? (
        ...
        <p className="text-center text-sm text-muted-foreground">This ticket is closed. Change status to reply.</p>
      )}
    </div>
  )
}
